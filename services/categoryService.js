...
exports.createCategory = async ({ user_id, name, category_type }) => {
  const query = "INSERT INTO category (user_id, name, category_type) VALUES ($1, $2, $3 ) RETURNING *";
...
exports.updateCategoryInDb = async (id, ...rest) => {
  ...
  const query = "UPDATE category SET user_id = $1, name = $2, category_type = $3 WHERE id = $4 RETURNING *";
  const params = [user_id, name, category_type, id];
  return await runQuery(query, params);
};

exports.deleteCategoryById = async (id) => {
  return await runQuery(`DELETE FROM category WHERE id = $1 RETURNING *`, [id]);
};
